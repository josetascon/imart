...
int main(int argc, char *argv[])
{
    ...
    std::string file_output = "./moving_demonsloi_";
    bool verbose;
    bool plot;
    unsigned int dim; int iter;
    double alpha;
    double tolerance, step;
    double sigma_f, sigma_e, sigma_g;
    ...
    ("alpha,a", po::value<double>(&alpha)->default_value(0.5), "Alpha weigth of image vs directional gradient. If 1.0 only gradient")
    ("sigma_gradient,g", po::value<double>(&sigma_g)->default_value(2.5), "Sigma gradient");
    ...
    auto img_fixed_norm = image_cpu<type>::new_pointer(dim);
    img_fixed->read(file_fixed);

    // std::string file_moving = argv[2];
    auto img_moving = image_cpu<type>::new_pointer(dim);
    auto img_moving_norm = image_cpu<type>::new_pointer(dim);
    ...
    // *img_fixed_norm = normalize<type>(*img_fixed);
    // *img_moving_norm = normalize<type>(*img_moving);
    ...
    auto demons1 = demons_loi<type,vector_cpu<type>>::new_pointer(img_fixed, img_moving, trfm);
    demons1->set_alpha(alpha);
    demons1->set_sigma_gradient(sigma_g);
    ...
    registro->set_levels_iterations(std::vector<int>{iter,iter,iter,iter});
    ...
    // output->print_data();
    // moving_warped->write("./moving_demons_warped.nii");
