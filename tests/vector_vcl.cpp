...
* @Last Modified time: 2020-06-22 11:46:55
*/
...
template <typename T>
class test_vector_vcl : public ::testing::Test{};
...
// ============================================
TYPED_TEST_CASE(test_vector_vcl, test_types);

TYPED_TEST(test_vector_vcl, constructor)
{
    vector_vcl<TypeParam> vec1;
    ASSERT_TRUE("vector_vcl" == vec1.get_name());
    ASSERT_EQ( 0, vec1.size() );

    vector_vcl<TypeParam> vec2(0);
    ASSERT_EQ( 0, vec2.size() );

    vector_vcl<TypeParam> vec3(121);
    ASSERT_TRUE("vector_vcl" == vec3.get_name());
    ASSERT_EQ( 121, vec3.size() );

    vector_vcl<TypeParam> vec4(1,3.0);
    ASSERT_FLOAT_EQ( 3.0, vec4[0] );

    vector_vcl<TypeParam> vec5(10,-5.2);
    ASSERT_TRUE("vector_vcl" == vec5.get_name());
    ASSERT_FLOAT_EQ( -5.2, vec5[0] );
    ASSERT_FLOAT_EQ( -5.2, vec5[4] );
    ASSERT_FLOAT_EQ( -5.2, vec5[9] );

    vector_vcl<TypeParam> vec6(vec5);
    ASSERT_TRUE("vector_vcl" == vec6.get_name());
    ASSERT_FLOAT_EQ( -5.2, vec5[0] );
    ASSERT_FLOAT_EQ( -5.2, vec5[4] );
    ASSERT_FLOAT_EQ( -5.2, vec5[9] );
}
...
//          Testing Pointers
// ============================================
TYPED_TEST(test_vector_vcl, pointers)
{
    using vgpu_pointer = typename vector_vcl<TypeParam>::pointer;
    vgpu_pointer vec1 = vector_vcl<TypeParam>::new_pointer();
    ASSERT_TRUE("vector_vcl" == vec1->get_name());
    ASSERT_EQ( 0, vec1->size() );

    vgpu_pointer vec2 = vector_vcl<TypeParam>::new_pointer(0);
    ASSERT_EQ( 0, vec2->size() );

    vgpu_pointer vec3 = vector_vcl<TypeParam>::new_pointer(201);
    ASSERT_TRUE("vector_vcl" == vec3->get_name());
    ASSERT_EQ( 201, vec3->size() );

    vgpu_pointer vec4 = vector_vcl<TypeParam>::new_pointer(1,-7.0);
    ASSERT_FLOAT_EQ( -7.0, vec4->operator[](0) );

    vgpu_pointer vec5 = vector_vcl<TypeParam>::new_pointer(21,11.2231);
    ASSERT_TRUE("vector_vcl" == vec5->get_name());
...
// ============================================
TYPED_TEST(test_vector_vcl, clones)
{
    using vgpu_pointer = typename vector_vcl<TypeParam>::pointer;
    vgpu_pointer vec1 = vector_vcl<TypeParam>::new_pointer(8,3.14159);
    vgpu_pointer vec2 = vec1->clone();
    vgpu_pointer vec3 = vec1->copy();
    vgpu_pointer vec4 = vec1->mimic();

    ASSERT_TRUE("vector_vcl" == vec2->get_name());
    ASSERT_TRUE("vector_vcl" == vec3->get_name());
    ASSERT_TRUE("vector_vcl" == vec4->get_name());
...
// ============================================
TYPED_TEST(test_vector_vcl, initialization)
{
    int size = 50;
    using vgpu_pointer = typename vector_vcl<TypeParam>::pointer;
    vgpu_pointer vec1 = vector_vcl<TypeParam>::new_pointer(size);
...
// ============================================
TYPED_TEST(test_vector_vcl, vector_operations)
{
    using vgpu_pointer = typename vector_vcl<TypeParam>::pointer;
    vgpu_pointer vec1 = vector_vcl<TypeParam>::new_pointer(30,2.1);
    vgpu_pointer vec2 = vector_vcl<TypeParam>::new_pointer(30,1.1);
    vgpu_pointer vec3;
    vgpu_pointer vec4 = vector_vcl<TypeParam>::new_pointer(30,4.2);
    vgpu_pointer vec5;
    vgpu_pointer vec6 = vector_vcl<TypeParam>::new_pointer(30,14.2123);
    vgpu_pointer vec7;
    vgpu_pointer vec8 = vector_vcl<TypeParam>::new_pointer(30,-2*14.2123);
    vgpu_pointer vec9;
    vgpu_pointer vec10 = vector_vcl<TypeParam>::new_pointer(30,4.0);
    vgpu_pointer vec11;
    ...
    vec7 = (*vec5) * (*vec6);
    ASSERT_TRUE("vector_vcl" == vec7->get_name());
    ...
    vec9 = (*vec7) / (*vec8);
    ASSERT_TRUE("vector_vcl" == vec9->get_name());
    ...
    vec11 = (*vec9) ^ (*vec10);
    ASSERT_TRUE("vector_vcl" == vec11->get_name());
...
// ============================================
TYPED_TEST(test_vector_vcl, scalar_operations)
{
    using vgpu_pointer = typename vector_vcl<TypeParam>::pointer;
    vgpu_pointer vec1 = vector_vcl<TypeParam>::new_pointer(14,1.5);
    vgpu_pointer vec2 = vector_vcl<TypeParam>::new_pointer(16,-4.632);
    vgpu_pointer vec3 = vector_vcl<TypeParam>::new_pointer(18,-12.0);
    vgpu_pointer vec4 = vector_vcl<TypeParam>::new_pointer(20,8.4);
    vgpu_pointer vec5 = vector_vcl<TypeParam>::new_pointer(22,2.0);
    vgpu_pointer vec6;
    vgpu_pointer vec7;
    vgpu_pointer vec8;
    vgpu_pointer vec9;
    ...
    ASSERT_TRUE("vector_vcl" == vec8->get_name());
    ...
    vec10 = (*vec5) ^ (3.0);
    ASSERT_TRUE("vector_vcl" == vec10->get_name());
...
// ============================================
TYPED_TEST(test_vector_vcl, reduction_operations)
{
    int size = 32;
    using vgpu_pointer = typename vector_vcl<TypeParam>::pointer;
    vgpu_pointer vec1 = vector_vcl<TypeParam>::new_pointer(size,0.0);
    vgpu_pointer vec2 = vector_vcl<TypeParam>::new_pointer(size,0.0);
...
// ============================================
TYPED_TEST(test_vector_vcl, auxialiary_functions)
{
    int size = 32;
    using vgpu_pointer = typename vector_vcl<TypeParam>::pointer;

    // cast function
    vgpu_pointer vec1 = vector_vcl<TypeParam>::new_pointer(size,-1.1);

    vector_vcl<int>::pointer vec2 = vec1->template cast<int>();
    vector_vcl<float>::pointer vec3 = vec1->template cast<float>();
    vector_vcl<double>::pointer vec4 = vec1->template cast<double>();
    vector_vcl<unsigned int>::pointer vec5 = vec1->template cast<unsigned int>();
    // vec2->print_data();
    ...
    // normalize function
    vgpu_pointer vec11 = vector_vcl<TypeParam>::new_pointer(size,0.0);
    vgpu_pointer vec12 = vector_vcl<TypeParam>::new_pointer(size,0.0);
    vgpu_pointer vec13;
    vgpu_pointer vec14;
