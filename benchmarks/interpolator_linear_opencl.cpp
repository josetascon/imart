...
static void bm_ilinear_ocl_2d(benchmark::State& state)
{
    // Perform setup here
    using type = float;     //4 Bytes
    int N = state.range(0); 

    auto image0 = image_ocl<type>::new_pointer(N,N);
    auto image1 = image_ocl<type>::new_pointer();
    image0->random();
    auto x0 = grid_ocl<type>::new_pointer(image0);
    auto x1 = x0->mimic();

    image_ocl<type>::pointer params(new image_ocl<type>{1.1, 0.1, -0.2, 0.9, 1.3, 8.0});
    auto taffine = affine_ocl<type>::new_pointer(2, params);
    auto interp0 = ilinear_ocl<type>::new_pointer(image0);
...
static void bm_ilinear_ocl_3d(benchmark::State& state)
{
    // Perform setup here
    using type = float;     //4 Bytes
    int N = state.range(0); 

    auto image0 = image_ocl<type>::new_pointer(N,N,N);
    auto image1 = image_ocl<type>::new_pointer();
    image0->random();
    auto x0 = grid_ocl<type>::new_pointer(image0);
    auto x1 = x0->mimic();

    image_ocl<type>::pointer params(new image_ocl<type>{1.1, 0.1, -0.2, 0.05, 1.2, 0.03, 0, -0.04, 1, 11.324, 201.4, 8.0});
    auto taffine = affine_ocl<type>::new_pointer(3, params);
    auto interp0 = ilinear_ocl<type>::new_pointer(image0);
...
BENCHMARK(bm_ilinear_ocl_2d)->Apply(CustomArguments2d);
BENCHMARK(bm_ilinear_ocl_3d)->Apply(CustomArguments3d);
