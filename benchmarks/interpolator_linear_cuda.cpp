...
* @Last Modified time: 2021-01-24 00:26:50
*/
...
static void bm_ilinear_cuda_2d(benchmark::State& state)
{
    ...
    auto taffine = affine_cuda<type>::new_pointer(2, params);
...
static void bm_ilinear_cuda_3d(benchmark::State& state)
{
    ...
    auto taffine = affine_cuda<type>::new_pointer(3, params);
    auto interp0 = ilinear_cuda<type>::new_pointer(image0);
...
BENCHMARK(bm_ilinear_cuda_2d)->Apply(CustomArguments2d);
BENCHMARK(bm_ilinear_cuda_3d)->Apply(CustomArguments3d);
