...
* @Last Modified time: 2020-07-20 15:55:18
*/
...
// Function to be timed
static void bm_sum_image_gpu_2d(benchmark::State& state)
{
    // Perform setup here
    using type = float;     //4 Bytes
    int N = state.range(0);
...
// Function to be timed
static void bm_sum_image_gpu_3d(benchmark::State& state)
{
    // Perform setup here
    using type = float;     //4 Bytes
    int N = state.range(0);
...
BENCHMARK(bm_sum_image_gpu_2d)->RangeMultiplier(10)->Range(10, 10000);
BENCHMARK(bm_sum_image_cpu_3d)->RangeMultiplier(10)->Range(10, 400);
BENCHMARK(bm_sum_image_gpu_3d)->RangeMultiplier(10)->Range(10, 400);
