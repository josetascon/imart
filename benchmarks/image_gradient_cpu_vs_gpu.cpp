...
* @Last Modified time: 2021-02-18 13:25:47
*/
...
// Function to be timed
static void bm_gradient_image_opencl_2d(benchmark::State& state)
{
    // Perform setup here
    using type = float;     //4 Bytes
    int N = state.range(0);
    ...
    {
        // This code gets timed
        image_opencl<type>::vector a = gradient(img1);
    };
    // std::cout << "Image size: " << img1.get_total_elements() << std::endl;
};

// Function to be timed
static void bm_gradient_image_opencl_3d(benchmark::State& state)
{
    // Perform setup here
    using type = float;     //4 Bytes
    int N = state.range(0);
...
BENCHMARK(bm_gradient_image_opencl_2d)->RangeMultiplier(10)->Range(10, 10000);
BENCHMARK(bm_gradient_image_cpu_3d)->RangeMultiplier(10)->Range(10, 400);
BENCHMARK(bm_gradient_image_opencl_3d)->RangeMultiplier(10)->Range(10, 400);
